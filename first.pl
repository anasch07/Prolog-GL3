...
% est_age_de(X, Y) denotes that X is Y years old
est_age_de(X, Y) :-
    personne(X, _, Y, _).

% habite_e(X, Y) denotes that X lives in the city Y
habite_e(X, Y) :-
    personne(X, _, _, Y).
...
% meme_age(X, Y) denotes that X and Y are the same age
meme_age(X, Y) :-
    personne(X, _, Age, _),
    personne(Y, _, Age, _),
    X \= Y.

% habite_la_meme_ville(X, Y) denotes that X and Y live in the same city
habite_la_meme_ville(X, Y) :-
    personne(X, _, _, Ville),
    personne(Y, _, _, Ville),
    X \= Y.

% epoux_possible(X, Y) denotes that X is a possible spouse for Y
epoux_possible(X, Y) :-
    masculin(X),
    feminin(Y),
    habite_la_meme_ville(X, Y),
